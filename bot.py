...
def get_advice_for_day(days):
    if days <= 3:
        return "Только начинаешь. Первые 72 часа самые тяжёлые — мозг требует дофамин. Это ломка, она пройдёт. Держись, брат."
    elif days <= 7:
    ...
    try:
        ...
        if save:
            ...
            if isinstance(user, dict):
                ...
                if len(user["message_ids"]) > 100:
                    user["message_ids"] = user["message_ids"][-100:]
                data[uid] = user
                save_data(data)
        return msg
...
async def midnight_cleanup(context):
    ...
    if deleted > 0:
        logger.debug(f"Удалено {deleted} сообщений для {chat_id}")
...
async def start_command(update, context):
    ...
    if user.get("active", False):
        days = get_days_since_start(chat_id)
        if days > 0:
            await send_message(context.bot, chat_id, f"Ты уже начал. Держишься {format_days_text(days)}. Продолжай.", save=False)
        else:
            await send_message(context.bot, chat_id, "Ты уже начал. Держись.", save=False)
        return
...
async def handle_hold(update, context):
    ...
    if last_date != current_date.isoformat():
        user["hold_count_today"] = 0
        user["last_hold_date"] = current_date.isoformat()
    ...
    if last_time:
        try:
            ...
            delta_seconds = (current_time - last_dt).total_seconds()
            if delta_seconds < 1800:
                mins = int((1800 - delta_seconds + 59) // 60)
                if mins == 1:
    ...
    user["last_hold_time"] = current_time.isoformat()
    user["last_hold_date"] = current_date.isoformat()
    ...
    for uid in active_users:
        if uid != chat_id:
            push_tasks.append(context.bot.send_message(uid, "✊"))
...
async def handle_heavy(update, context):
    await update.message.reply_text("Я здесь, брат.\nЧто тебе нужно прямо сейчас?", reply_markup=get_heavy_keyboard())
...
async def handle_breakdown_response(update, context):
    ...
    await update.message.reply_text(
        f"{responses.get(text, 'Ты сделал шаг вперёд, сказав об этом.')}\n\nНачинаем с чистого листа. Я с тобой.",
        reply_markup=get_main_keyboard()
    )
...
async def handle_text_message(update, context):
    ...
    if len(text) > 8 and text not in ["🔥 Упражнение", "🧠 Что происходит с телом", 
                                       "🔄 Другое упражнение", "↩ Назад"]:
        await send_message(
            context.bot, chat_id,
            "Понимаю, брат. Тяжко.\n"
...
async def restore_jobs_on_startup(application):
    ...
    for user_id in active_users:
        try:
            ...
            logger.error(f"Ошибка восстановления задач для {user_id}: {e}")
...
def main():
    ...
    hold_conv = ConversationHandler(
        ...
        states={
            REFLECTION: [MessageHandler(filters.TEXT & ~filters.COMMAND, handle_reflection)]
        },
        fallbacks=[],
    ...
    breakdown_conv = ConversationHandler(
        ...
        states={
            BREAKDOWN_STATE: [MessageHandler(filters.TEXT & ~filters.COMMAND, handle_breakdown_response)]
        },
        fallbacks=[],
        conversation_timeout=300
    )

...
def main_menu_keyboard():
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("📊 Дни", callback_data="days"),
         InlineKeyboardButton("👋 Ты тут?", callback_data="tut")],
...
def heavy_menu_keyboard():
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("💪 Помочь себе", callback_data="help"),
         InlineKeyboardButton("😅 Почти сорвался", callback_data="almost")],
...
def help_menu_keyboard():
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("🔄 Ещё способ", callback_data="more_tip")],
        [InlineKeyboardButton("↩️ Назад", callback_data="back")]
    ])

# ==================== Основные функции ====================
async def send_menu(bot, chat_id, user):
    """Отправляет или обновляет меню 'че как?'"""
    text = "че как?"
    ...
    if not user.get("menu_message_id"):
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if state == "normal":
        if cb == "hold":
            await handle_hold(chat_id, context, user)
        elif cb == "heavy":
            user["state"] = "heavy_menu"
            save_data(data)
            await query.message.edit_reply_markup(reply_markup=heavy_menu_keyboard())
        elif cb == "days":
            ...
            await query.message.edit_text("че как?\n\n" + msg, reply_markup=main_menu_keyboard())
        elif cb == "tut":
            await query.message.edit_text(
                "че как?\n\n" + random.choice(TU_TUT_FIRST) + " " + random.choice(TU_TUT_SECOND),
                reply_markup=main_menu_keyboard()
            )
        elif cb == "thanks":
            await query.message.edit_text(
    ...
    # ==================== Меню 'тяжело' ====================
        ...
        if cb == "help":
            ...
            save_data(data)
            await query.message.edit_text(f"че как?\n\n{tip}", reply_markup=help_menu_keyboard())
        elif cb == "almost":
            await query.message.edit_text("че как?\n\nНу ебать, держись, займись чем-нибудь.", reply_markup=heavy_menu_keyboard())
        elif cb == "fail":
            reset_streak(chat_id)
            user["state"] = "normal"
            save_data(data)
            await query.message.edit_text("че как?\n\nНичего страшного.\nНачнём заново. Ты молодец, что сказал честно.", reply_markup=main_menu_keyboard())
        elif cb == "back":
            user["state"] = "normal"
            user["used_tips"] = []
            save_data(data)
            await query.message.edit_text("че как?", reply_markup=main_menu_keyboard())
...
async def handle_hold(chat_id, context, user):
    data, user_data = get_user(chat_id)
    today = NOW().date()
    last_date = user_data.get("hold_date")
    last_time = user_data.get("hold_time")
    count = user_data.get("hold_count", 0)
    ...
    if last_time:
        if (NOW() - datetime.fromisoformat(last_time)).total_seconds() < 1800:
            ...
            await context.bot.send_message(chat_id, f"Погоди ещё {minutes_left} минут, брат.")
            return
    if count >= 5:
        await context.bot.send_message(chat_id, "Сегодня это уже 5 раз, брат, тормози. Завтра сможешь отправить еще. ✊")
        return
    await context.bot.send_message(chat_id, random.choice(HOLD_RESPONSES))
    for uid in [int(k) for k,v in load_data().items() if v.get("active")]:
        if uid != chat_id:
            try:
                ...
                pass
    user_data["hold_time"] = NOW().isoformat()
    user_data["hold_date"] = str(today)
    user_data["hold_count"] = count + 1
    save_data(data)
    await send_menu(context.bot, chat_id, user_data)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await send_menu(context.bot, chat_id, user)

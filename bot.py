...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await send_message(context.bot, chat_id,
        ...
        "Все, кто тоже в деле, получат пуш. Просто узнают, что ты ещё здесь и не сдаешься.\n"
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "▶ Начать":
        ...
        await send_message(context.bot, chat_id, random.choice(["Держусь.", "Все будет нормально.", "Всё по плану.", "Сегодня полегче.", "Я рядом."]))
    elif text == "✊ Держусь":

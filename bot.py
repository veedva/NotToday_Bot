...
HELP_TECHNIQUES = [
    "Садись и дыши так по кругу: вдох носом 4 секунды → задержи дыхание считая до 4 → выдох ртом 4 секунды → снова не дыши 4 секунды. Повтори 6–8 раз подряд. Через минуту мозг переключается и тяга уходит, проверено тысячу раз.",
...
TU_TUT_SECOND = ["Держимся.", "Я с тобой.", "Всё по плану?", "Не хочу сегодня.", "Сегодня не буду.", "Я рядом.", "Держись.", "Все будет нормально."]
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "▶ Начать":
        ...
        days = get_days(chat_id)
        msg = "Первый день." if days == 0 else f"Прошло {days} дней." if days > 1 else "Прошёл 1 день."
        await send(context.bot, chat_id, msg)
    elif text == "❤️ Спасибо":
        await send(context.bot, chat_id, "Спасибо, брат. ❤️\n\nЕсли хочешь поддержать:\nСбер 2202 2084 3481 5313\n\nГлавное — держись.", save=False)
    elif text == "⏸ Пауза":

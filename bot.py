...
MORNING_MESSAGES = [
    ...
    "Доброе! Давай сегодня без этого.", "Утро. Денег жалко да и ну его.", "Привет. Сегодня легко обойдёмся.",
    "Братан, доброе. Сегодня точно нет.", "Эй. Сегодня не в тему.", "Доброе утро. Только не сегодня.",
    "Привет. Может завтра, но сегодня нет.", "Утро. Сегодня спокойно обходимся.", "Эй. Сегодня не стоит."
]
...
HELP_TECHNIQUES = [
    "Дыши так: вдох носом 4 сек → задержи 4 → выдох ртом 4 → задержи 4. 6–8 раз. Тяга уходит за минуту — работает всегда.",
...
async def handle_hold_button(chat_id, context):
    ...
    if count >= 5:
        await send_message(context.bot, chat_id, "Сегодня это уже 5 раз. ✊\nЗавтра снова сможешь.")
        return

    await send_message(context.bot, chat_id, random.choice([
        "Отправлено, молодец! ✊", "Горжусь! ✊", "Красава. ✊", "Держись! ✊", "Так держать. ✊"
    ]), save_for_deletion=False)
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "▶ Начать":
        ...
        await send_message(context.bot, chat_id, random.choice(["Не сдаюсь.", "Держусь.", "Я не буду.", "Мы справимся.", "Я рядом."]))
    elif text == "✊ Держусь":

...
MORNING_MESSAGES = [
    ...
    "Доброе утро. Не, точно не сегодня.",
    "Привет. Держишься?",
    "Доброе утро. Сегодня много дел, наверное я нет заранее скажу, окей?",
    "Привет. Сегодня обойдёмся без этого всего надеюсь.",
    "Доброе утро. Все будет хорошо сегодня.",
...
TU_TUT_FIRST = [
    ...
    "Да, да.", "Здарова карова епта", "Ага.", "Здравствуй.", "Тут, не переживай."
]

TU_TUT_SECOND = [
    "Держимся.", "Я с тобой.", "Всё по плану.", "Не хочу сегодня.", "Сегодня не буду.",
    "Я рядом.", "Держись.", "Всё будет нормально.", "Че как ты, лошара?", "Играю."
]
...
Холод активирует блуждающий нерв — это моментально снижает тревогу и тягу. 
Метод работает за 60-90 секунд.""",

    """🫁 Дыхание 4-7-8
Вдох на 4 секунды → задержка на 7 → выдох на 8.
Повтори 4 раза. Этот паттерн снижает кортизол на 20-30% и успокаивает нервную систему.""",

    """⏱ Волновая теория
Тяга приходит волнами — поднимается, достигает пика и спадает.
Поставь таймер на 5 минут и просто наблюдай. 90% волн проходят сами за это время.""",

    """🚪 Смена контекста
Встань и выйди в другую комнату или на улицу.
Нейронные связи «место → желание» разрываются при смене обстановки.""",

    """🍋 Вкусовой переключатель
Кусок лимона, имбиря или острого перца.
Резкий вкус «перебивает» дофаминовый сигнал в мозге, давая передышку.""",

    """✊ Физический выброс
Сожми кулаки изо всех сил на 10 секунд → отпусти. 5 раз.
Мышечное напряжение поглощает психическое, адреналин уходит в мышцы.""",

    """💧 Рефлекс ныряльщика
Умой лицо ледяной водой или опусти лицо в таз с холодной водой на 30 секунд.
Замедляет пульс на 10-20%, мгновенное успокоение.""",

    """📝 Логический барьер
Напиши на бумаге 3 причины, почему именно сейчас не стоит.
Включает префронтальную кору — рациональную часть мозга, которая принимает решения.""",

    """🌿 Кислородный коктейль
10 медленных глубоких вдохов животом.
Кислород снижает адреналин, возвращает чувство контроля.""",

    """💪 Физический вызов
Планка 45-60 секунд до дрожи в мышцах.
Пока тело борется — мозг забывает про тягу. Эндорфины после упражнения заменят дофамин.""",

    """🚶 Движение как лекарство
Быстрая прогулка 7-10 минут на улице.
Вырабатывает BDNF — природный антидепрессант, восстанавливает нейроны.""",

    """👀 Метод 5-4-3-2-1
Назови: 5 вещей которые видишь → 4 которые трогаешь → 3 которые слышишь → 2 запаха → 1 вкус.
...
Перезагружает вегетативную нервную систему, снимает напряжение.""",
    ...
    """🥜 Биохимическая стабилизация
Горсть орехов, кусок сыра или яйцо. Ёбни белка короче.
...
Физический выброс адреналина через руки, отвлекает мозг.""",
    ...
    """🦸 Поза силы
2 минуты: ноги широко, руки в боки, грудь вперёд, подбородок вверх.
Повышает тестостерон, снижает кортизол, меняет биохимию в пользу уверенности.""",

    """🤔 Метод HALT
Спроси себя: Голоден? Злой? Одинок? Устал?
Исправь хотя бы один пункт — тяга ослабнет на 50-70%.""",

    """🌊 Серфинг по тяге
Представь тягу как океанскую волну. Не борись — просто наблюдай со стороны как она поднимается и спадает.
...
Пока тело в стрессе от нагрузки — мозг забывает про дофаминовый голод.""",
    ...
    """🎵 Аудио-перезагрузка
3 минуты любимой музыки на полной громкости в наушниках.
Звук напрямую воздействует на эмоциональные центры мозга, меняя состояние.""",

    """🧘 Медитация наблюдения
Сядь, закрой глаза, просто наблюдай за мыслями как за облаками.
Не вовлекайся, не оценивай. Через 5 минут тяга уйдёт сама.""",

    """📖 Книжный шок
Прочитай 2 страницы сложной книги или статьи.
Нагрузи префронтальную кору — она забудет про желание, занятая обработкой информации."""
]
...
def get_heavy_keyboard():
    return ReplyKeyboardMarkup([
        [KeyboardButton("💪 Упражнение"), KeyboardButton("🧠 Информация")],
...
async def handle_heavy(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "💪 Упражнение — быстрая техника для снятия тяги\n"
        "🧠 Информация — научные факты и методы\n"
        "💔 Срыв — ну ты сам понимаешь\n\n"
        "Помни: Тяга пройдёт. Всегда проходит.",
...
async def handle_info_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "🤯 Искажения — ошибки мышления при ломке\n"
